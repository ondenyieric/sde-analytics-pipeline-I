...
def _run_dbt(command: str, failure_metric: str | None = None) -> None:
    result = subprocess.run(
        [
            "bash",
            "-lc",
            f"cd {DBT_PROJECT_DIR} && {command} --profiles-dir {DBT_PROJECT_DIR}",
        ],
        check=False,
        capture_output=True,
        text=True,
    ...
    ingest = BashOperator(
        ...
        env={
            **os.environ,
            ...
            "POSTGRES_DB": os.environ.get("POSTGRES_DB", "pipeline"),
        ...
        try:
            ...
            for line in text.splitlines():
                if not line.startswith("kafka_consumergroup_lag{"):
                    continue
                if (
                    'topic="pipeline.' not in line
                    or 'consumergroup="clickhouse_' not in line
                ):
                    continue
                try:
