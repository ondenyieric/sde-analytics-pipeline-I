        ...
        insert into `mart`.`fct_orders__dbt_backup`
        ("order_id", "order_date", "user_id", "product_id", "category", "quantity", "revenue", "order_ts")-- MergeTree, partitioned by month (toYYYYMM(order_date)) to keep partitions
...
inner join `mart`.`dim_products` as p
